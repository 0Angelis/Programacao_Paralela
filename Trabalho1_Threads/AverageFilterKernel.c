...
{
  int tid,ini,fim,linTotal,colTotal,tamFiltro;
} tipoPack;
...
void *filtroThread(void *ptr){
  ...
  //printf("Thread %i: Executando da linha (%i-%i).\n",pack->tid, pack->ini, pack->fim);

  for(i=pack->ini; i<=pack->fim; i++)
  {
    for (j = 0; j < pack->colTotal; j++)
    {
      d = 0;
      e = 0;
...
PGM *filtroParalelo(PGM *pgm, int filt_size)
{
  ...
  {
    pgmParalelo->data[i] = malloc(pgm->width * sizeof(unsigned char));
  }
...
int main()
{
  ...
  if (openPGM(pgm, ch))
  {
    ...
    if (choice && 1)
    {
      ...
      printf("\nTempo de execucao Filtro Paralelo: %f", paraleloFiltrado->time);
    ...
    {
      printf("Tamanho de janela invalido\n");
      exit(EXIT_FAILURE);
    }
  }
  free(pgm->data);
